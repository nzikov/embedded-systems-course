...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	{
	  angle_x = atan2f(ax, sqrtf(ay*ay + az*az)) * 180.0f / M_PI;
	  angle_y = atan2f(ay, sqrtf(ax*ax + az*az)) * 180.0f / M_PI;
	}
  ...
  while (1)
  {
	  if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) && !(checkbutt)) checkbutt = 1;

	  LSM303_Read();
	  Convert_Accel();
	  Calculate_Angles();
	  if(checkbutt && tim>5){
		  sprintf(buffer, "AX=%.3f g | ANG_X=%.2f | AY=%.3f g | ANG_Y=%.2f\r\n", ax, angle_x, ay, angle_y);
	  	  send(buffer);
	  	  tim = 0;
	  }
	  tim++;
	  if(angle_x>0){
		  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, angle_x*10);
		  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 0);
	  }
	  if(angle_x<0){
		  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, 0);
		  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, abs(angle_x)*10);
	  }
	  HAL_Delay(100);
