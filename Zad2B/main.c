...
/* USER CODE BEGIN PV */
uint8_t checkstat = 1; // variable for check which program must be using
/* USER CODE END PV */
...
void set_pwm(uint8_t pwm1,uint8_t pwm2){
	if(pwm1 > 99 || pwm1 < 0) pwm1 = 0;
	if(pwm2 > 99 || pwm2 < 0) pwm2 = 0;
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, pwm1*10);
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, pwm2*10);
}
  ...
  while (1)
  {
	  ...
	  for (int i=0; i<checkstat; i++){
	  			  set_pwm(i, checkstat-i);
	  			  HAL_Delay(100);
	  		  }
