...
int main(void)
{
 ...
 uint8_t acc_buf[6];
  ...
  int16_t L3GD20_ReadZ()
  {
   uint8_t zl = L3GD20_Read(0x2C);
   uint8_t zh = L3GD20_Read(0x2D);
  ...
  L3GD20_Write(0x23, 0x00);
  char buffer[64];
    HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1);
    uint8_t id;
    id = L3GD20_Read(0x0F);
    sprintf(buffer, "ID = 0x%X\r\n", id);
    send(buffer);
    float gyro_z_dps;
    int16_t raw;
  ...
  {
   raw = L3GD20_ReadZ();
   gyro_z_dps = raw*0.00875f;
   angle += gyro_z_dps * 0.1f;
   uint16_t pwm = abs((int)gyro_z_dps) * 4;
   if(pwm > 999) pwm = 999;

__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, pwm);
	if(flag && counter > 4){
		sprintf(buffer, "Z = %.2f dps | ANG = %.2f deg\r\n", gyro_z_dps, angle);
		send(buffer);
		counter = 0;
	}
	else counter++;;
